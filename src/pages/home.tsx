...
function FloatingOrb({ delay, size, color, x, y }: { delay: number; size: number; color: string; x: string; y: string }) {
  return (
    <motion.div
      ...
      style={{ width: size, height: size, background: color, left: x, top: y, filter: "blur(50px)" }}
      animate={{ y: [0, -28, 0], scale: [1, 1.1, 1], opacity: [0.16, 0.28, 0.16] }}
      ...
      >
        {items.map((item, i) => (
          <span key={i} className="font-display font-bold text-xs tracking-widest text-primary/70 flex items-center gap-4">
            {item}<span className="text-primary/25">◆</span>
          </span>
        ))}
      </div>
    </div>
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="relative w-full h-52 rounded-xl border border-white/10 bg-black/50 overflow-hidden select-none">
              ...
              {particles.map(p => (
                <div key={p.id} className="absolute rounded-full pointer-events-none"
                  style={{ width: Math.max(3, 7 * p.life), height: Math.max(3, 7 * p.life), left: `calc(${p.x * 100}% - ${Math.max(1.5, 3.5 * p.life)}px)`, top: `calc(${p.y * 100}% - ${Math.max(1.5, 3.5 * p.life)}px)`, background: p.color, opacity: p.life, boxShadow: `0 0 ${6 * p.life}px ${p.color}`, transform: `scale(${0.5 + p.life * 0.5})` }} />
              ))}
          ...
          <div className="lg:col-span-2 rounded-xl border border-white/8 bg-white/[0.02] p-4">
            ...
            <Link href="/ai-agent" className="mt-4 block text-center font-mono text-[9px] text-primary/40 hover:text-primary/70 transition-colors tracking-widest">
              VIEW FULL SIMULATOR →
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {TECHS.map((group, gi) => (
            <motion.div key={group.category}
              ...
              transition={{ duration: 0.45, delay: gi * 0.08, ease: [0.22, 1, 0.36, 1] }}
...
export default function Home() {
  ...
  const springY = useSpring(yText, { stiffness: 80, damping: 22 });
  ...
  return (
    <main className="min-h-screen pt-16 flex flex-col">
      ...
      <section ref={heroRef} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden border-b border-white/5">
        ...
        <HeroCanvas />
        <FloatingOrb delay={0}   size={340} color="radial-gradient(circle, hsla(127,65%,52%,0.28), transparent)" x="8%"  y="18%" />
        <FloatingOrb delay={1.5} size={280} color="radial-gradient(circle, hsla(263,68%,58%,0.22), transparent)" x="68%"  y="52%" />
        <FloatingOrb delay={3}   size={220} color="radial-gradient(circle, hsla(48,95%,58%,0.18), transparent)"  x="53%"  y="8%"  />
        <FloatingOrb delay={2}   size={200} color="radial-gradient(circle, hsla(360,100%,71%,0.13), transparent)" x="84%" y="12%" />
        <div className="absolute inset-0 scanline z-0 opacity-[0.12] pointer-events-none" />
        <div className="absolute inset-0 z-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at center, transparent 38%, rgba(0,0,0,0.65) 100%)" }} />
        <div className="absolute inset-0 z-0 pointer-events-none opacity-[0.035]" style={{ backgroundImage: "linear-gradient(hsl(127 49% 60%) 1px, transparent 1px), linear-gradient(90deg, hsl(127 49% 60%) 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
          ...
          <motion.div className="max-w-3xl mx-auto flex flex-col items-center">
            ...
            >
              <div className="relative">
                <div className="absolute inset-0 rounded" style={{ background: "hsl(127 49% 60% / 0.35)", animation: "pulse-ring 2s ease-out infinite" }} />
                <Link
                  href="/play"
                  onClick={playShoot}
            ...
            >
              <motion.div
                ...
                className="w-6 h-6 mx-auto border-r-2 border-b-2 border-primary/40 rotate-45"
              />
            </motion.div>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className="absolute bottom-0 left-0 right-0 bg-black/40 border-t border-primary/15 backdrop-blur-md z-10 py-3 hidden md:block"
        >
          <div className="container mx-auto px-4 flex justify-between items-center font-mono text-xs text-primary/80">
